...
from .ai_matching import trigger_ai_matching
import requests
...
class MinistryDashboardViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if hospital_id:
            ...
            successful_surgeries = surgeries_qs.filter(status='successful').count()
            success_percentage   = round((successful_surgeries / total_surgeries) * 100, 2) if total_surgeries > 0 else 0

            latest_successful_qs = Surgery.objects.filter(
                hospital=h, status__in=['successful', 'failed']
            ...
            latest_successful = [
                {
                    ...
                    "status":         s.status,
                    "patient_name":   f"{s.organ_matching.patient.first_name} {s.organ_matching.patient.last_name}" if s.organ_matching and s.organ_matching.patient else None,
                    "birthdate":      s.organ_matching.patient.birthdate.strftime('%Y-%m-%d') if s.organ_matching and s.organ_matching.patient and s.organ_matching.patient.birthdate else None,
                    "created_at":     s.created_at.strftime('%Y-%m-%d %H:%M:%S'),
                }
                for s in latest_successful_qs
            ] if latest_successful_qs.exists() else []

            all_organs   = [choice[0] for choice in OrganType.choices]
            organs_stats = Surgery.objects.filter(hospital=h, organ_matching__isnull=False) \
                .values('organ_matching__organ_type').annotate(count=Count('id'))
            organs_dict  = {o['organ_matching__organ_type']: o['count'] for o in organs_stats}
            organs_needed = [{"organ": organ, "count": organs_dict.get(organ, 0)} for organ in all_organs]
        ...
        successful_surgeries = Surgery.objects.filter(status='successful').count()
        ...
        for h in hospitals:
            surgeries_qs              = Surgery.objects.filter(hospital=h)
            surgeries_count           = surgeries_qs.count()
            successful_surgeries_count = surgeries_qs.filter(status='successful').count()

            latest_successful_qs = Surgery.objects.filter(
                hospital=h, status__in=['successful', 'فشلت']
            ).order_by('-created_at')[:3]
            ...
            organs_needed      = Surgery.objects.filter(hospital=h, organ_matching__isnull=False) \
                .values('organ_matching__organ_type').annotate(count=Count('organ_matching__organ_type'))
            organs_dict        = {o['organ_matching__organ_type']: o['count'] for o in organs_needed}
            organs_needed_list = [{"organ": organ, "count": organs_dict.get(organ, 0)} for organ in all_organs]

            hospital_alerts_data = [
                {
                    "id":            a.id,
                    "message":       a.message,
                    "message_title": a.message_title,
                ...
                for a in AlertHospital.objects.filter(hospital=h).order_by('-created_at')
            ]
            ...
            hospitals_data.append({
                ...
                "hospital_alerts":             hospital_alerts_data,
        ...
        for organ in all_organs:
            count            = Surgery.objects.filter(organ_matching__organ_type=organ).count()
            successful_count = Surgery.objects.filter(organ_matching__organ_type=organ, status='successful').count()
            organ_data.append({
                "organ":              organ,
                "count":              count,
                "percentage":         round((count / total_surgeries) * 100, 2) if total_surgeries > 0 else 0,
                "successful_count":   successful_count,
                "success_percentage": round((successful_count / count) * 100, 2) if count > 0 else 0,
            })
        ...
        monthly_stats = Surgery.objects.filter(created_at__gte=start_date) \
            ...
            .values('month') \
            .annotate(total=Count('id'), successful=Count('id', filter=Q(status='successful')))

        stats_dict   = {(s['month'].year, s['month'].month): s for s in monthly_stats}
        monthly_data = []
        for i in range(6):
            ...
            successful = stat['successful'] if stat else 0
            monthly_data.append({
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                return Response(
                    {"message": "فشل إنشاء المريض", "error": e.message_dict if hasattr(e, 'message_dict') else e.messages},
                    status=status.HTTP_400_BAD_REQUEST
                )
            except Exception as e:
                return Response(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                return Response(
                    {"message": "فشل إنشاء المتبرع", "error": e.message_dict if hasattr(e, 'message_dict') else e.messages},
                    status=status.HTTP_400_BAD_REQUEST
                )
            except Exception as e:
                return Response(
                    {"message": "فشل إنشاء المتبرع", "error": str(e)},
                    status=status.HTTP_400_BAD_REQUEST
                )
            token, _ = Token.objects.get_or_create(user=user)
            trigger_ai_matching(user)
